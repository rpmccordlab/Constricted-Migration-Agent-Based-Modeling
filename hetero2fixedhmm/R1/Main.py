...
proportion = float(sys.argv[2]) # = bottom / (bottom + top) # For altering the ratio of bottom:top
...
search_rad = max([high_params['cell_rad'],low_params['cell_rad']])
search_rad += max([high_params['max_speed'],low_params['max_speed']])
...
writer.writerow(['Top','Bottom','High','Low','TxH','TxL','BxH','BxL'])
...
for timepoint in range(num_timepoints):
    ...
    rows = [cell_comps.count('top'),cell_comps.count('bottom'),cell_ids.count('high'),cell_ids.count('low')]
    rows += [freq[('top','high')],freq[('top','low')],freq[('bottom','high')],freq[('bottom','low')]]
